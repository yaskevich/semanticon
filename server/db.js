...
const getStats = async () => {
  ...
  const lenObj = {};
  for (const x of resultPhrases.rows) {
    const len = String(x.phrase.length);
    if (lenObj?.[len]) {
      lenObj[len] += 1;
    } else {
      lenObj[len] = 1;
  ...
  for (const x in exprsObject) {
    // console.log(exprsObject[x]);
    for (const tid of exprsObject[x]) {
      if (occObj?.[tid]) {
        occObj[tid] += 1;
      } else {
        occObj[tid] = 1;
  ...
  const sortedOcc = Object.entries(occObj).sort((a, b) => b[1] - a[1]);
  // const globalPercent = 0;

  let top = 0;

  for (const x of sortedOcc) {
